...
outcomes_layoff = outcomes[pd.notnull(outcomes['layoff'])]
...
# for i in range(0, len(outcomes_layoff)):
# 	index = outcomes_layoff.challengeID.values[i]
# 	layoffs[index] = outcomes_layoff.layoff.values[i]
# test_ids = list()
# for i in range(0, 4243):
# 	if (i not in outcomes_layoff.challengeID.values):
# 		test_ids.append(i)
# print(test_ids)
# test_ids.pop(0)
# test_data = df.loc[df['challengeID'].isin(test_ids)]
# train_data =  df.loc[df['challengeID'].isin(outcomes_layoff.challengeID)]
# clf = RandomForestClassifier(n_estimators = 100,criterion = "gini", max_depth = 10)
# start_time = time.time()
# test_outcomes = [int(i) for i in outcomes_layoff.layoff]
...
# 	layoffs[test_ids[i]] = rfpred_test[i]

# layoffs.pop(0)
# for i in range(0, len(layoffs)):
# 	if (layoffs[i] == 0):
# 		layoffs[i] = False
# 	else:
# 		layoffs[i] = True
...
# rows = zip(layoffs)
# import csv
# with open('predict_layoff.csv', 'wb') as f:
...
# Get rows of training data that have reported layoff (exclude those with NA)


df = df.loc[df['challengeID'].isin(outcomes_layoff.challengeID)]
...
target = [int(i) for i in outcomes_layoff.layoff]
...
# # outcomes_layoff = [int(i) for i in outcomes_layoff.challengeID]
# # # bcv = Bootstrap(n = len(outcomes_layoff))
...
# # # 	y_train = [outcomes_layoff[i] for i in index_train]
# # # 	y_test = [outcomes_layoff[i] for i in index_test]
...
# plt.title('ROC Curves for Classifiers (Layoff)')
