...
# ##################### FOR PREDICTIONS ############################
# jobTrainings = [None]*4243
# print("CONDUCTING PREDICTION")

# for i in range(0, len(outcomes_jobTraining)):
# 	index = outcomes_jobTraining.challengeID.values[i]
# 	jobTrainings[index] = outcomes_jobTraining.jobTraining.values[i]
# test_ids = list()
# for i in range(0, 4243):
# 	if (i not in outcomes_jobTraining.challengeID.values):
# 		test_ids.append(i)
# print(test_ids)
# test_ids.pop(0)
# test_data = df.loc[df['challengeID'].isin(test_ids)]
# train_data =  df.loc[df['challengeID'].isin(outcomes_jobTraining.challengeID)]
# clf = RandomForestClassifier(n_estimators = 100,criterion = "gini", max_depth = 10)
# start_time = time.time()
# test_outcomes = [int(i) for i in outcomes_jobTraining.jobTraining]
...
# 	jobTrainings[test_ids[i]] = rfpred_test[i]

# jobTrainings.pop(0)
# for i in range(0, len(jobTrainings)):
# 	if (jobTrainings[i] == 0):
# 		jobTrainings[i] = False
# 	else:
# 		jobTrainings[i] = True
# print(jobTrainings)

# rows = zip(jobTrainings)
# import csv
# with open('predict_jobTraining.csv', 'wb') as f:
...
df = df.loc[df['challengeID'].isin(outcomes_jobTraining.challengeID)]
...
# plt.title('ROC Curves for Classifiers (jobTraining)')
