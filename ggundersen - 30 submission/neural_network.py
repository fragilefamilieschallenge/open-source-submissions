...
import numpy as np
from models.neural_network import nnutils
...
class NeuralNetwork(object):
    ...
    def __init__(self, layers, biases=None, weights=None):
        ...
        self.biases = biases if biases else nnutils.init_biases(layers)
        self.weights = weights if weights else nnutils.init_weights(layers)
...
# -----------------------------------------------------------------------------
    ...
    def train(self, X, y, rate):
        ...
        for x, y in zip(X, y):
            ...
            add = lambda a, b: a + b
            nabla_b_sum = nnutils.apply(nabla_b_sum, nabla_b, add)
            nabla_W_sum = nnutils.apply(nabla_W_sum, nabla_W, add)

        num_samples = X.shape[0]
        step = lambda a, b: a - (rate / num_samples) * b
        self.biases = nnutils.apply(self.biases, nabla_b_sum, step)
        self.weights = nnutils.apply(self.weights, nabla_W_sum, step)
...
# -----------------------------------------------------------------------------
    ...
    def _backpropagate(self, x, y):
        ...
        delta_b = (activations[-1] - y) * nnutils.sigmoid_prime(zeds[-1])
        ...
        for j in range(len(self.layers)-2, 0, -1):
            W = self.weights[j]
            z = zeds[j]
            delta_b = np.dot(W, delta_b) * nnutils.sigmoid_prime(z)
