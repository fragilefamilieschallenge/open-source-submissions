...
for pfi in range(6):
    ...
    finish = clock()
    pca_train_overhead = finish - start
    start = clock()
    X_test_pca = pca.transform(X_test_std)
    finish = clock()
    pca_test_overhead = finish - start

    lrModel = LinearRegression()
    start = clock()    
    lrModel.fit(X_train_pca, y_train)
    finish = clock()
    traintime['lr_pca'] = (finish - start) + pca_train_overhead
    start = clock()
    y = lrModel.predict(X_test_pca)
    finish = clock()
    testtime['lr_pca'] = (finish - start) + pca_test_overhead
    [result_row] = y.shape
    #print y
    sumsum = 0
    for i in range(result_row):
    ...
    finish = clock()
    traintime['lr_std'] = (finish - start)
    start = clock()      
    y = lrModel.predict(X_test_std)
    finish = clock()
    testtime['lr_std'] = (finish - start)
    [result_row] = y.shape
    #print y
    sumsum = 0
    for i in range(result_row):
    ...
    finish = clock()
    traintime['ridge_pca'] = (finish - start) + pca_train_overhead    
    start = clock()
